...
create_OnetDAG <- function(K = 5, nC = 1, network = "prefattach", trimKmax = NULL) {
  ...
  # --------------------------------------------------------------------------------------------
  gen_network_k_reg <- function(n, K, ...) {
    if (n <= 200) K <- 5
  ...
  D <- set.DAG(D, latent.v = c("LatentW", "probY"), n.test = 200)
}
...
compute_stats_interventions <- function() {
  ...
  datO <- sim(D, n = 50000, rndseed = net.seed, rndseed.reset.node = "LatentW")
  ...
  datO <- sim(D, n = 25000, rndseed = net.seed, rndseed.reset.node = "LatentW")
  head(datO)
  head(datO$W1)
  ...
  # datFull <- sim(Dset, actions="plus.nF.PA", n = 25000,  rndseed = net.seed, rndseed.reset.node = "LatentW")[["plus.nF.PA"]]
  datFull <- sim(Dset, actions="plus.nF.PA.alt", n = 25000,  rndseed = net.seed, rndseed.reset.node = "LatentW")[["plus.nF.PA.alt"]]
...
runsim <- function(nsim, Sim_params, psi0, n_replpsi0) {
  ...
  run_tmlenet <- function(datO, NetInd_mat, sW, sA, K, Qform, hforms, ATE = TRUE, intervene1.sA, intervene2.sA, psi0, bin.method, maxNperBin) {
    ...
    extract_ests <- function(estimator, res) {
      ...
      # IC_based Vars & CIs conditional on W
      ...
      condW.CIlen <- as.vector(condW.CIs[,2]-condW.CIs[,1]); names(depCIlen) <- rownames(CIs)
      ...
      return(list(depVar.Est = depVar.Est, bootVar.Est = bootVar.Est, iidVar.Est = iidVar.Est,
                  depCIcov = CIcov, bootCIcov = bootCIcov, iidCIcov = iidCIcov,
                  depCIlen = depCIlen, bootCIlen = bootCIlen, iidCIlen = iidCIlen,
                  MAD = MAD, est = est, aux.vars = aux.vars
                  ))
    }
...
run_all_sims <- function() {
  ...
  # sW <-  def_sW(netW2 = W2[[1:5]], W1, W2, WNoise, corrW.F1, corrW.F2, corrW.F3, corrW.F4, corrW.F5, HUB = ifelse(nF >= 25, 1, 0), PA0 = (PA == 0))
...
# Hence all the intervention summaries preserve exactly the same order of evaluation as in obs.sW.sA;
  ...
  # Regression formulas
  # ----------------------------------------------------------------------
  # Qforms <- list(Q.corr = "Y ~ nF.PA + A.PAeq0 + nFPAeq0.PAeq1 + sum.net.A + sum.net.A.sum.netPA + PA0 + W1 + W2 + corrW.F1 + corrW.F2 + corrW.F3 + corrW.F4 + corrW.F5",
  Qforms <- list(Q.corr = "Y ~ nF.PA + A.PAeq0 + nFPAeq0.PAeq1 + sum.net.A + sum.net.A.sum.netPA + PA + W1 + W2 + corrW.F1 + corrW.F2 + corrW.F3 + corrW.F4 + corrW.F5",
  ...
  # ----------------------------------------------------------------------
  sW.nFPA <-  def_sW(W1, W2, WNoise, corrW.F1, corrW.F2, corrW.F3, corrW.F4, corrW.F5, HUB = ifelse(nF >= 25, 1, 0)) +
              def_sW(A) +
  ...
  sW.nFPA.A <-  def_sW(W1, W2, WNoise, corrW.F1, corrW.F2, corrW.F3, corrW.F4, corrW.F5, HUB = ifelse(nF >= 25, 1, 0))
 ...
 merge_All_Sim_Params <- function(network = "prefattach") {
  ...
  # All_Sims_Params <- merge_All_Sim_Params(network = "prefattach")
  # All_Sims_Params <- merge_All_Sim_Params(network = "smallworld")
