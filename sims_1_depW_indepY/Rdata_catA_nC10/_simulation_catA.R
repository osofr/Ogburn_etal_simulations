...
create_OnetDAG <- function(Kmax = 5, nC = 1) {
  ...
  # D <- D + network("NetInd_k", Kmax = Kmax, netfun = "gen_network_k_reg_indClusters", nC = nC)
  D <- D +
    # W1 - iid categorical confounder (6 categories, 1-6):
    ...
    node("net.A", distr = "rconst", const = ifelse(nF>0, sum(new.A[[1:Kmax]])/nF, 0)) +
    node("net.A.cat", distr = "rconst", const =
      (net.A<=1)*1 + (net.A>1 & net.A<=2)*2 + (net.A>2 & net.A<=3)*3 +
      (net.A>3 & net.A<=4)*4 + (net.A>4 & net.A<=5)*5 +
      (net.A>5 & net.A<=6)*6 + (net.A>6 & net.A<=7)*7) +
      # (net.A>7 & net.A<=0.8)*8 + (net.A>0.8 & net.A<=0.8)*9 +
      # (net.A>0.9 & net.A<=1)*10) +

    node("A.gstar", distr = "rconst", const = ifelse(A + shift <= Amax, A + shift, A)) +
    node("net.A.gstar", distr = "rconst", const = ifelse(nF>0, sum(A.gstar[[1:Kmax]])/nF, 0)) +
    node("net.A.cat.gstar", distr = "rconst", const =
      (net.A.gstar<=1)*1 + (net.A.gstar>1 & net.A.gstar<=2)*2 + (net.A.gstar>2 & net.A.gstar<=3)*3 +
      (net.A.gstar>3 & net.A.gstar<=4)*4 + (net.A.gstar>4 & net.A.gstar<=5)*5 +
      (net.A.gstar>5 & net.A.gstar<=6)*6 + (net.A.gstar>6 & net.A.gstar<=7)*7) +

    node("probY", distr = "rconst",
        const = plogis(-0.10*new.A +
...
dagO <- create_OnetDAG(Kmax = 80)
datO <- sim(dagO, n = 100000)
# head(datO[,c("A", "net.A", "net.A.cat", "A.gstar", "net.A.gstar", "net.A.cat.gstar")], 500)
# datO[,c("A", "net.A", "net.A.cat", "A.gstar", "net.A.gstar", "net.A.cat.gstar")]
sort(unique(datO[,"net.A"])); sort(unique(datO[,"net.A.cat"]));
sort(unique(datO[,"net.A.gstar"])); sort(unique(datO[,"net.A.cat.gstar"]));

# dagO <- create_OnetDAG(Kmax=5, nC=20)
# datO <- sim(dagO, n=100)
# attributes(datO)$netind_cl$NetInd
...
create_OnetDAG <- function(Kmax = 5, nC = 1) {
  ...
  D <- DAG.empty()
  D <- D + network("NetInd_k", Kmax = 5, netfun = "gen_network_k_reg")
  ...
  D <- D +
      node("probY", distr = "rconst",
          const = plogis(-0.10*new.A +
                        ...
                        -0.5*ifelse(nF>0, sum(W2[[1:Kmax]])/nF, 0) +
                        +0.5*(W1-1) - 0.58*W2 +
                        +0.5*corrW.F1 + 0.5*corrW.F2 + 0.5*corrW.F3 + 0.5*corrW.F4 + 0.5*corrW.F5
                        ),
          replaceNAw0 = TRUE) +
      node("Y", distr = "rbern", prob = probY)
  D <- set.DAG(D)
  ...
  cor(datO[,"W2"], datO[,"W2"][Net_mat[,1]])
  cor(datO[,"corrW.F1"], datO[,"corrW.F1"][Net_mat[,1]])
  cor(datO[,"corrW.F2"], datO[,"corrW.F2"][Net_mat[,2]])
  cor(datO[,"corrW.F3"], datO[,"corrW.F3"][Net_mat[,3]])
  cor(datO[,"corrW.F4"], datO[,"corrW.F4"][Net_mat[,4]])
  cor(datO[,"corrW.F5"], datO[,"corrW.F5"][Net_mat[,5]])
...
runsim <- function(nsim, Sim_params, psi0, n_forpsi0, n_replpsi0) {
  ...
  # --------------------------------------------------------------------------------------------
  ex <- Filter(function(x) is.function(get(x, .GlobalEnv)), ls(.GlobalEnv)) # ex <- c(ex, "nsamp"); # print("ex: "); print(ex)
...
run_all_sims <- function() {
  ...
  def_sW <- def.sW(W1, W2, W3) +
            ...
            def.sW(W1.eq1 = W1==1, W1.eq2 = W1==2, W1.eq3 = W1==3, W1.eq4 = W1==4)
            # def.sW(W1.W2 = W1*W2) +
  ...
  def_sA <- def.sA(A, net.A = ifelse(nF > 0, rowSums(A[[1:Kmax]])/nF, 0), replaceNAw0 = TRUE)
  # Qforms <- list(Q.corr = "Y ~ W1 + W2 + W3 + A + net.mean.A + net.mean.W1", #Q.corr = "Y ~ W1 + W2 + W3 + A + net.mean.A",
  Qforms <- list(Q.corr = "Y ~ W1 + W2 + W3 + net.W2 + A + net.A", #Q.corr = "Y ~ W1 + W2 + W3 + A + net.mean.A",
                 Q.miss = "Y ~ W3")
...
use_sims_plot_byK <- function() {
  ...
  # nsamp_vec <- c(500, 1000, 5000, 10000, 20000, 30000, 40000, 50000)
  # Kmax_vec <- seq(5, 30, by=5)
  nsamp_vec <- unique(unlist(lapply(simRes_all, function(sim) sim[['Sim_params']][['nsamp']])))
  Kmax_vec <- unique(unlist(lapply(simRes_all, function(sim) sim[['Sim_params']][['Kmax']])))

  # Q&h-correct
  # test plot for one scenario:
  simRes_byscen <- simRes_all[["N50000.Kmax30"]]
  model_names <- names(simRes_byscen[["sims_MeanRes_byscen"]])
  print(names(simRes_byscen))
  print(simRes_byscen[["Sim_params"]])
  print(simRes_byscen[["sims_MeanRes_byscen"]])
  make_ggplot_hist(model_scen = model_names[1], simRes_byscen = simRes_byscen)

  # loop over all Kmax for a fixed N:
  for (nsamp in nsamp_vec) {
    ggplothist_byK <- lapply(Kmax_vec, function(Kmax)
        make_ggplot_hist(model_scen = model_names[1], simRes_byscen = simRes_all[["N"%+%nsamp%+%".Kmax"%+%Kmax]]))
    # plot histograms by K:
    pdf(file=outdir%+%"fig_byKmax_N" %+% nsamp %+%".pdf",width=7,height=12) # pdf
    multiplot(plotlist = ggplothist_byK, cols = 1)
    dev.off()
  }

  # loop over all N for fixed Kmax:
  for (Kmax in Kmax_vec) {
    ggplothist_byK <- lapply(nsamp_vec, function(nsamp)
        make_ggplot_hist(model_scen = model_names[1], simRes_byscen = simRes_all[["N"%+%nsamp%+%".Kmax"%+%Kmax]]))
    # plot histograms by K:
    pdf(file=outdir%+%"fig_byN_Kmax" %+% Kmax %+%".pdf",width=30,height=8) # pdf
    multiplot(plotlist = ggplothist_byK, cols = length(ggplothist_byK))
    dev.off()
  }
