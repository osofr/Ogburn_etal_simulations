...
run_all_sims <- function() {
  ...
  nsamp_vec <- c(500, 1000, 5000, 10000, 20000, 30000, 50000)
  ...
  Qforms <- list(Q.corr = "Y ~ A + net.A + W1 + W2 + corrW.F1 + corrW.F2",
                 Q.miss = "Y ~ A + W1 + W2 + WNoise")

  hforms <- list(h.corr = c(hform.g0 = "A + net.A ~ W1 + W2 + corrW.F1 + corrW.F2",
                            hform.gstar = "A + net.A ~ W1 + W2 + corrW.F1 + corrW.F2"),
  ...
  Common_Sim_Params <- list(shift = 1, nComm = 10, def_sW = def_sW, def_sA = def_sA, nobsBin_frac = c(0.0025, 0.005), bin.method = "equal.mass", Qforms = Qforms, hforms = hforms)
